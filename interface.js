...
$(document).keydown(function(e){
  ...
  commitstate(cleared); 
  $(document).trigger('log', ['clearShortcut', {key: 'a'}]);
  }
  else if(key == 's'){
    hidepopups();
    var state = currentstate();
    ...
    $(document).trigger('log', ['checkShortcut', {key: 's'}]);
  } else if (key == 'd'){
    flippage(-1);
    $(document).trigger('log', ['prevShortcut', {key: 'd'}]);
  } else if (key == 'f'){
    flippage(1);
    $(document).trigger('log', ['nextShortcut', {key: 'f'}]);
  }
});
...
$(document).keydown(function(e) {
    ...
    if ( $.inArray(e.keyCode,[49,50,51,52]) !== -1) 
    {
      var state = currentstate();
      if (state.puzzle[focusElementCell] !== null) return;
      $("#sn"+focusElementCell).html(e.keyCode-48);
        state.answer[focusElementCell] = e.keyCode;
        state.work[focusElementCell] = 0;
        // redraw(state, focusElementCell);
        // setcurnumber(0);
        // setTimeout(function() {
        //   commitstate(state);
        // }, 0);
    $(document).trigger('log', ['numbShortcut', {key: e.keyCode-48}]);
    }
    else if(e.which == 8){
      var state = currentstate();
      if (state.puzzle[focusElementCell] !== null) return;
      state.answer[focusElementCell] = null;
      state.work[focusElementCell] = 0;
      $("#sn"+focusElementCell).html('');
      $(document).trigger('log', ['backspace', {key: 'erase'}]);
      // redraw(state, focusElementCell);
      // setcurnumber(0);
      // setTimeout(function() {
      //   commitstate(state);
...
$(document).keydown(function(e){
  var state = currentstate();
  if(e.which == 38){
    $(document).trigger('log', ['arrowUp', {key: 'up'}]);
    if(![0,1,2,3].includes(focusElementCell)){
      focusElement(-4);
    }else{
      focusElement(12);
    }
  } else if(e.which == 40){
    $(document).trigger('log', ['arrowDown', {key: 'down'}]);
    if(![12,13,14,15].includes(focusElementCell)){
      focusElement(4);
    }else{
      focusElement(-12);
    }
  } else if(e.which== 39){
    $(document).trigger('log', ['arrowRight', {key: 'right'}]);
    if(focusElementCell !=15){
      focusElement(1);
    }else{
      focusElement(-15);
    }
  } else if(e.which == 37) {
    $(document).trigger('log', ['arrowLeft', {key: 'left'}]);
    if(focusElementCell != 0){
      focusElement(-1);
    }
    else{
      focusElement(15);
...
function focusElement(change) {
  let previousBox = '#sc' + focusElementCell;
  $(previousBox).find('div.sudoku-border').toggleClass('focuselement',false);
  focusElementCell += change;
  let presentBox = '#sc' + focusElementCell;
  $(presentBox).find('div.sudoku-border').toggleClass('focuselement',true);
}
